...
object DeleteFaceFromFaceList {
	def deleteFaceFromFaceList(listId : String, picId : String) : String = {
	// **********************************************
	...
	val uriBase : String = "https://westeurope.api.cognitive.microsoft.com/face/v1.0/facelists/" + listId + "/persistedFaces/" + picId;
		...
		try {
			...
			if (entity != null) {
				...
				if (EntityUtils.toString(entity).isEmpty()){
					return("Picture " + picId + " was deleted successfully!")
				}
				else{
